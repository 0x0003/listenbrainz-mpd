...
*MPD_HOST*::
  ...
  May be a hostname or a bare IP address.
  IPv6 addresses should not be enclosed in square brackets.
  To use a server password, specify a value like _password_**@**_host_.
  If the host starts with **/** it indicates the path to a Unix socket.
  If the host starts with **@** it indicates the name of an abstract socket (Linux only).
